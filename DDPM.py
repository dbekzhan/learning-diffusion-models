...
Based on the paper "Denoising Diffusion Probabilistic Models" by Ho et al. (2020)
https://arxiv.org/abs/2006.11239

Key improvements over base DDPM:
- Local attention mechanism for efficient computation
- Position embeddings from transformer architectures
- Cosine annealing learning rate schedule
- Classifier-free guidance (from Hang et al., 2022)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 10 == 0:
            ...
            for class_label in range(10):
                ...
                sample = diffusion.sample(model, n=4, labels=class_labels, cfg_scale=3)
                samples.append(sample)
            samples = torch.cat(samples, dim=0)
            save_samples(samples, f"samples/epoch_{epoch}.png")
            
            real_batch = next(iter(dataloader))[0]
            fake_batch = diffusion.sample(model, n=batch_size)
